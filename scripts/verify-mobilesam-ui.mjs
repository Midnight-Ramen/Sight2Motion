...
try {
 ...
 await page.getByLabel('Selection mode',{exact:true}).selectOption('negative');
 await page.mouse.click(rect.x+off+(481-40)*scale,rect.y+100*scale);
 await expect(page.getByText(/Negative points: 1/)).toBeVisible({timeout:180000});
 await expect(page.getByRole('button',{name:'Undo point',exact:true})).toBeEnabled({timeout:180000});
 await page.getByRole('button',{name:'Undo point',exact:true}).click();
 await expect(page.getByRole('button',{name:'Lock target',exact:true})).toBeEnabled({timeout:180000});
 await expect(page.getByText(/Negative points: 0/)).toBeVisible();
 await page.screenshot({path:'test-results/mobilesam-mirrored.png'});
 await page.getByRole('button',{name:'Lock target',exact:true}).click();
 await expect(c).toHaveCount(0);
 await expect(page.getByText(/Tracking: Jonathan · Detector: person/)).toBeVisible();
 ...
 await writeFile('test-results/mobilesam-ui.json',JSON.stringify({logs,requests},null,2));
